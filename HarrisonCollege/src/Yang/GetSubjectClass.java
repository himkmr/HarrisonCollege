 ...
 */
@WebServlet("/GetSubjectClass")
public class GetSubjectClass extends HttpServlet {
	...
	// Get Current Class in a subject			
				TypedQuery<Hclass> q = DBUtil.createQuery("SELECT h FROM Hclass h where h.semester = ?1 and h.year = ?2 and h.hcours.subject = ?3",Hclass.class)
						.setParameter(1, currentSemester).setParameter(2, currentYear).setParameter(3, subjectName);
